...
{
    /// <summary>Segredo vazio/ausente — trava DESLIGADA (comportamento legado). Preserva
    /// dev local e ambientes sem gateway configurado.</summary>
    LegacyBypass,
...
/// Cópia por serviço do mesmo padrão da <c>Fifa2026.V2.Functions.Infrastructure</c> (mesma
/// convenção de duplicação de helper self-contained que o repo já usa em
/// <c>CategoryLabelMapper</c>). O gateway continua o guardião ÚNICO do JWT (ADE-004
/// preservada): isto NÃO valida token, só prova a ORIGEM (que o gateway chamou).
/// </summary>
internal static class GatewayKeyValidator
{
    ...
    {
        // Fail-open condicionado ao segredo: vazio = trava desligada = legado (sem gateway).
